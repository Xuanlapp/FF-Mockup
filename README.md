...
Standalone FF Mockup desktop app with an automatic local PSD worker. The worker claims `waiting` jobs, supports local storage and public image URLs (including Google Drive), and runs up to five jobs concurrently.
...
- Background local worker with automatic startup and GitHub Releases auto-update support
...
The first release is `v0.0.1`. Future releases must use a higher semantic version so Electron can update installed copies automatically.
